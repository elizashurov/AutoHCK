...
require_relative 'functest_tools'
...
module AutoHCK
  ...
  class FunctestEngine
    ...
    def initialize(project)
      ...
      prepare_extra_sw
      # Functest connects directly to the client VM over WinRM;
      # ensure it gets a world-facing NIC (must happen before QemuHCK is created).
      @project.options.common.client_world_net = true
    ...
    def self.platform(logger, options)
      ...
      unless File.exist?(platform_json)
        raise InvalidConfigFile, "Platform configuration not found: #{platform_json}"
      end
    ...
    # ResultReport and JUnit generators call these on every engine.
    # Functest manages its own results; return empty collections so the
    # shared report infrastructure does not crash.
    def test_steps
      []
    end
    ...
    def run
      ...
      ResourceScope.open do |scope|
        ...
        executor = Functest::TestExecutor.new(@project, client.tools,
    ...
    def load_tests(loader)
      if @project.options.test.category
        suite = loader.load_suite(@project.options.test.category)
        @logger.info("Loaded test suite: #{suite['name']}")
        loader.load_suite_tests(suite)
      elsif @project.options.test.testcase
        @project.options.test.testcase.split(',').map do |test_name|
    ...
    def load_engine_config
      ...
      @logger.error("Failed to load engine config: #{e.message}")
      {}
    end
